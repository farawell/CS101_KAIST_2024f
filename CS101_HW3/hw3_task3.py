...
def main():
    ...
    print(add_transaction(20241000, -8000))
    print(add_transaction(20245000, -10000))
    print(add_transaction(20245000, -10000))
...
def add_transaction(student_ID, amount):
    ### START OF YOUR CODE (Task 3-1) ###
    global Users_DB
    ...
    if (len(str(student_ID)) != 8) \
       or (int(str(student_ID)[4]) not in range(0, 6)) \
       or not str(amount).replace('-', '', 1).isdigit() \
       or student_ID not in set(Users_DB.keys()):
       return False

    student_type = Users_DB[student_ID]['type']
    final_amount = amount

    if amount < 0:
        discount_factor = 1 - (0.1 * (2 - student_type))
        final_amount = round(amount * discount_factor)
        if Users_DB[student_ID]['balance'] < abs(final_amount): return False
        
    Users_DB[student_ID]['balance'] += final_amount
    Transactions_DB.append((student_ID, student_type, amount, final_amount))
    return True
    ### END OF YOUR CODE ###
    
def detect_remove_bad_users():
    ### START OF YOUR CODE (Task 3-2) ###
    global Users_DB
    global Transactions_DB
    ...
    for user_id in Users_DB.keys():
        true_balance = sum(final_amount for (student_ID, _, _, final_amount) in Transactions_DB if user_id == student_ID)
        if true_balance != Users_DB[user_id]['balance']:
            bad_users_list.append(user_id)

    for user_id in bad_users_list:
